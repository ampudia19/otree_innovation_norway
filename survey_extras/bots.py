...
def gen_random_data(field_name, player_class, player, field_values: Dict[str, object] = None):
...
class AutoBot:
    ...
    def play_round(self):
        assert hasattr(self, 'PlayerClass'), 'AutoBot needs to be used as a mixin, ' \
                                             'e.g. class PlayerBot(Bot, AutoBot)'
        ...
        field_values = random.choice(self.custom_field_values) if hasattr(self, 'custom_field_values') else None
