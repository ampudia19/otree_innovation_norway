...
class SurveyPage(Page):
    ...
    def _go_to_page(self, page_index: int) -> abstract.FormPageOrInGameWaitPage:
        # sanity checks
        assert 1 <= page_index <= self.participant._max_page_index, 'page_index out of range'
    ...
    def _progress(self) -> Union[List[ProgressItem], None]:
        ...
        default_progress = int(100 * (self.participant._index_in_pages / self.participant._max_page_index))
    ...
    PLAYER_INTERNAL_FIELDS = ['id', 'id_in_group', '_payoff', 'participant',
                              'session', 'round_number', 'subsession', 'group']

    @property
    def all_player_fields(self):
        """All but internal fields, in order of declaration"""
        return [f.name for f in self.player._meta.fields if f.name not in self.PLAYER_INTERNAL_FIELDS]
